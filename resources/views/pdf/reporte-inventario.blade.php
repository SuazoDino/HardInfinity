    ...
    <div class="container">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                ...
                <div class="stat-value" style="font-size: 13px;">S/ {{ number_format($stats['valor_inventario'], 0) }}</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">Stock Bajo</div>
                <div class="stat-value" style="color: #dc2626;">{{ $stats['productos_bajo_stock'] }}</div>
            </div>
            <div class="stat-card">
                <div class="stat-label">Sin Stock</div>
                <div class="stat-value" style="color: #991b1b;">{{ $stats['productos_sin_stock'] }}</div>
            </div>
        </div>
        ...
        <div style="margin-top: 25px;">
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr class="{{ $producto->stock == 0 ? 'stock-bajo' : ($producto->stock < 10 ? 'stock-critico' : '') }}">
                        <td style="font-family: monospace; font-size: 8px;">{{ $producto->sku }}</td>
                        <td>{{ Str::limit($producto->name, 40) }}</td>
                        <td>{{ $producto->brand->name ?? 'N/A' }}</td>
                        <td>{{ $producto->category->name ?? 'N/A' }}</td>
                        <td class="text-right"><strong>{{ $producto->stock }}</strong></td>
                        <td class="text-right">S/ {{ number_format($producto->price, 2) }}</td>
                        <td class="text-right"><strong>S/ {{ number_format($producto->stock * $producto->price, 2) }}</strong></td>
                    </tr>
                    @endforeach
                </tbody>
                <tfoot>
                    <tr style="background-color: #f0f9ff; font-weight: bold; border-top: 2px solid #0ea5e9;">
                        ...
                        <td class="text-right">{{ $stats['stock_total'] }}</td>
                        <td></td>
                        <td class="text-right" style="color: #0ea5e9; font-size: 12px;">S/ {{ number_format($stats['valor_inventario'], 2) }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
